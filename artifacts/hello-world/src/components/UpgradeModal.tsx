...
export default function UpgradeModal({ visible, onClose, reason }: UpgradeModalProps) {
  ...
  const getPrice = (plan: typeof plans[0]) => {
    ...
    if (isIndia) {
      const price = billing === "yearly" ? plan.yearlyINR / 12 : plan.monthlyINR;
      return `₹${price}/mo`;
    } else {
      const price = billing === "yearly" ? plan.yearlyUSD / 12 : plan.monthlyUSD;
      return `$${price}/mo`;
    }
  };
            ...
            <div key={plan.name} style={{
              ...
              <div style={{
                ...
                <div style={{ textAlign: "right" }}>
                  <div style={{
                    fontSize: 18, fontWeight: 700, color: plan.color === C.border ? C.textMid : plan.color,
                    fontFamily: "'Montserrat', sans-serif",
                  }}>{getPrice(plan)}</div>
                  {getSavings(plan) && (
                    <div style={{
